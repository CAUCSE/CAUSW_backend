...
import net.causw.app.main.api.dto.userAcademicRecordApplication.CurrentUserAcademicRecordApplicationResponseDto;
import net.causw.app.main.api.dto.userAcademicRecordApplication.UpdateUserAcademicRecordApplicationStateRequestDto;
import net.causw.app.main.api.dto.userAcademicRecordApplication.UpdateUserAcademicStatusRequestDto;
import net.causw.app.main.api.dto.userAcademicRecordApplication.UserAcademicRecordApplicationInfoResponseDto;
import net.causw.app.main.api.dto.userAcademicRecordApplication.UserAcademicRecordApplicationListResponseDto;
...
import net.causw.app.main.api.dto.userAcademicRecordApplication.UserAcademicRecordListResponseDto;
...
public class UserAcademicRecordApplicationService {
	...
	public void exportUserAcademicRecordListToExcel(HttpServletResponse response) {
		...
		for (User user : userList) {
			List<UserAcademicRecordLog> userAcademicRecordLogList = userAcademicRecordLogRepository
	...
	public Page<UserAcademicRecordApplicationListResponseDto> getAllUserAwaitingAcademicRecordPage(Pageable pageable) {
		return userAcademicRecordApplicationRepository.findAllByAcademicRecordRequestStatus(pageable,
	...
	@Transactional
	public UserAcademicRecordInfoResponseDto updateUserAcademicRecordNote(String userId,
	...
	public UserAcademicRecordApplicationInfoResponseDto getUserAcademicRecordApplicationInfo(String userId,
		...
		if (!userAcademicRecordApplication.getUser().getId().equals(user.getId())) {
			throw new BadRequestException(ErrorCode.ROW_DOES_NOT_EXIST,
	...
	@Transactional
	public UserAcademicRecordInfoResponseDto updateUserAcademicStatus(User controllerUser,
	...
	public UserAcademicRecordApplicationResponseDto updateUserAcademicRecordApplicationStatus(
		...
		UserAcademicRecordApplication userAcademicRecordApplication = getUserAcademicRecordApplication(
			updateUserAcademicRecordApplicationStateRequestDto.getApplicationId());
		...
		AcademicRecordRequestStatus targetAcademicRecordRequestStatus = updateUserAcademicRecordApplicationStateRequestDto
			.getTargetAcademicRecordRequestStatus();

		if (targetAcademicRecordRequestStatus == AcademicRecordRequestStatus.REJECT) {
			// 거절 사유 저장
			userAcademicRecordApplication.setRejectMessage(
	...
	public UserAcademicRecordApplicationResponseDto createUserAcademicRecordApplication(
		...
		validCreateUserAcademicRecordApplicationRequestDto(createUserAcademicRecordApplicationRequestDto,
			imageFileList);
		...
		if (targetAcademicStatus == AcademicStatus.ENROLLED) {
			// 이미지 파일 저장
			List<UuidFile> uuidFileList = uuidFileService.saveFileList(imageFileList,
	...
	public CurrentUserAcademicRecordApplicationResponseDto getCurrentUserAcademicRecordApplication(User user) {
		...
		UserAcademicRecordApplication userAcademicRecordApplication = getRecentAwaitOrRejectUserAcademicRecordApplication(
			user);
	...
	private UserAcademicRecordApplication getUserAcademicRecordApplication(String applicationId) {
		return userAcademicRecordApplicationRepository.findById(applicationId)
			.orElseThrow(() -> new BadRequestException(ErrorCode.ROW_DOES_NOT_EXIST,
				MessageUtil.USER_ACADEMIC_RECORD_APPLICATION_NOT_FOUND));
	}

	private UserAcademicRecordApplication getRecentAwaitOrRejectUserAcademicRecordApplication(User user) {
		List<UserAcademicRecordApplication> awaitUserAcademicRecordApplicationList = userAcademicRecordApplicationRepository
			.findAllByAcademicRecordRequestStatusAndUser(
				AcademicRecordRequestStatus.AWAIT,
				user);
		...
		if (awaitUserAcademicRecordApplicationList.size() > 1) {
			throw new InternalServerException(ErrorCode.ROW_IS_DUPLICATED,
				MessageUtil.USER_ACADEMIC_RECORD_APPLICATION_DUPLICATED);
	...
	private static void validUpdateUserAcademicStatusRequestDto(
		UpdateUserAcademicStatusRequestDto updateUserAcademicStatusRequestDto) {
		...
		if (updateUserAcademicStatusRequestDto.getTargetAcademicStatus().equals(AcademicStatus.ENROLLED)) {
			if (updateUserAcademicStatusRequestDto.getTargetCompletedSemester() == null) {
				throw new BadRequestException(ErrorCode.INVALID_PARAMETER,
					MessageUtil.TARGET_CURRENT_COMPLETED_SEMESTER_NOT_EXIST);
			}
		}
		// 대상 사용자의 변경 타겟 학적 상태가 졸업인 경우, 졸업 년도와 졸업 유형이 필수 입력 값
			...
			if (updateUserAcademicStatusRequestDto.getTargetGraduationYear() == null
				|| updateUserAcademicStatusRequestDto.getTargetGraduationType() == null) {
				throw new BadRequestException(ErrorCode.INVALID_PARAMETER,
					MessageUtil.GRADUATION_INFORMATION_NOT_EXIST);
			}
		}
	}
	...
	private void validCreateUserAcademicRecordApplicationRequestDto(
		...
		// 대상 사용자의 변경 타겟 학적 상태가 졸업인 경우, 졸업 년도와 졸업 유형이 필수 입력 값
		else if (createUserAcademicRecordApplicationRequestDto.getTargetAcademicStatus()
			.equals(AcademicStatus.GRADUATED)) {
			if (createUserAcademicRecordApplicationRequestDto.getGraduationYear() == null
				|| createUserAcademicRecordApplicationRequestDto.getGraduationType() == null) {
				throw new BadRequestException(ErrorCode.INVALID_PARAMETER,
					MessageUtil.GRADUATION_INFORMATION_NOT_EXIST);
			}
		}
	...
	private UserAcademicRecordApplicationListResponseDto toUserAcademicRecordApplicationListResponseDto(
		UserAcademicRecordApplication userAcademicRecordApplication) {
		return UserAcademicRecordDtoMapper.INSTANCE.toUserAcademicRecordApplicationListResponseDto(
			userAcademicRecordApplication);
	}

	private UserAcademicRecordInfoResponseDto toUserAcademicRecordInfoResponseDto(User user,
		List<UserAcademicRecordLog> userAcademicRecordLogList) {
		UserAcademicRecordInfoResponseDto responseDto = UserAcademicRecordDtoMapper.INSTANCE
			.toUserAcademicRecordInfoResponseDto(
				user,
	...
	private UserAcademicRecordApplicationResponseDto toUserAcademicRecordApplicationResponseDto(
		UserAcademicRecordLog userAcademicRecordLog) {
		return UserAcademicRecordDtoMapper.INSTANCE.toUserAcademicRecordApplicationResponseDto(userAcademicRecordLog);
	}

	private UserAcademicRecordApplicationInfoResponseDto toUserAcademicRecordApplicationResponseDto(
		UserAcademicRecordApplication userAcademicRecordApplication) {
		return UserAcademicRecordDtoMapper.INSTANCE.toUserAcademicRecordApplicationInfoResponseDto(
			userAcademicRecordApplication);
	}

	private CurrentUserAcademicRecordApplicationResponseDto toCurrentUserAcademicRecordResponseDto(Semester semester,
		UserAcademicRecordApplication userAcademicRecordApplication, Boolean isRejected) {
		return UserAcademicRecordDtoMapper.INSTANCE.toCurrentUserAcademicRecordResponseDto(semester,
			userAcademicRecordApplication, isRejected);
	}
