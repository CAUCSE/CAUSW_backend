...
public record PostCreateRequest(
	...
	@Valid @Schema(description = "이미지 메타데이터 목록. 각 항목의 fileIndex는 'images' 파트의 파일 배열 인덱스와 매핑됩니다.") List<ImageCreateMeta> images) {
	...
	public record ImageCreateMeta(
		...
		@NotNull(message = "파일 인덱스를 입력해 주세요.") @Schema(description = "'images' 파트의 파일 배열 인덱스", example = "0") Integer fileIndex,

		@NotNull(message = "대표 이미지 여부를 입력해 주세요.") @Schema(description = "대표 이미지 여부 (목록에서 정확히 1개만 true)", example = "true") Boolean isRepresentative) {
	}
}
