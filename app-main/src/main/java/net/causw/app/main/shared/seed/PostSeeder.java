...
public class PostSeeder extends BasePostSeeder<PostSeeder.PostItem> {
	...
	private void loadBoardIds() {
		...
		// 예외 처리
		if (this.freeBoardId == null)
			throw new RuntimeException("자유게시판이 없습니다. BoardSeeder를 먼저 실행하세요.");
		if (this.humorBoardId == null)
			throw new RuntimeException("유머게시판이 없습니다. BoardSeeder를 먼저 실행하세요.");
	}
