...
public interface CeremonyMapper {
	...
	CeremonyDetailResult toDetailResult(Ceremony ceremony);

	// 내 경조사 상세 보기
	@InheritConfiguration(name = "toDetailResult")
	@Mapping(target = "state", source = "ceremonyState")
	@Mapping(target = "note", source = "note")
	CeremonyDetailResult toMyDetailResult(Ceremony ceremony);
	...
	@Mapping(target = "state", ignore = true)
	CeremonySummaryResult toSummaryResult(Ceremony ceremony);

	// 내 경조사 목록 조회
	@InheritConfiguration(name = "toSummaryResult")
	@Mapping(target = "state", source = "ceremonyState")
	CeremonySummaryResult toMySummaryResult(Ceremony ceremony);
	...
	@Mapping(target = "state", source = "ceremonyState")
	@Mapping(target = "category", source = ".", qualifiedByName = "mapCategory")
	CeremonyAdminListResult toAdminListResult(Ceremony ceremony);

	// 관리자 경조사 상세 조회
	@InheritConfiguration(name = "toDetailResult")
	@Mapping(target = "state", source = "ceremonyState")
	@Mapping(target = "note", source = "note")
	CeremonyDetailResult toAdminDetailResult(Ceremony ceremony);
