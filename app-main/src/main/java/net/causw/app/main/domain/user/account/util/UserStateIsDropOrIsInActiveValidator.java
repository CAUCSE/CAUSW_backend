package net.causw.app.main.domain.user.account.util;

import net.causw.app.main.domain.user.account.entity.user.User;
import net.causw.app.main.domain.user.account.enums.user.UserState;
import net.causw.app.main.shared.AbstractValidator;
import net.causw.global.exception.ErrorCode;
import net.causw.global.exception.UnauthorizedException;

public class UserStateIsDropOrIsInActiveValidator extends AbstractValidator {
	private final User user;
	private final UserState userState;

	private UserStateIsDropOrIsInActiveValidator(User user) {
		this.user = user;
		this.userState = user.getState();
	}

	public static UserStateIsDropOrIsInActiveValidator of(User user) {
		return new UserStateIsDropOrIsInActiveValidator(user);
	}

	@Override
	public void validate() {
		if (!(this.userState.equals(UserState.REJECT) || this.userState.equals(UserState.DROP)
			|| this.user.isInactive())) {
			throw new UnauthorizedException(
				ErrorCode.BLOCKED_USER,
				"등록된 사용자가 아닙니다.");
		}
	}
}
