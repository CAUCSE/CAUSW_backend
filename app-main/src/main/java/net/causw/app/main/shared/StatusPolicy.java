...
public class StatusPolicy {
	...
	public static boolean isDeletable(Comment comment, User user, Board board) {
		...
		if (comment.getIsDeleted())
			return false;
		return roles.contains(Role.ADMIN) || roles.contains(Role.PRESIDENT) || roles.contains(Role.VICE_PRESIDENT)
			|| comment.getWriter().getId().equals(user.getId());
	}
	...
	public static boolean isAdminOrPresident(User user) {
		return user.getRoles().contains(Role.ADMIN) || user.getRoles().contains(Role.PRESIDENT) || user.getRoles()
			.contains(Role.VICE_PRESIDENT);
	}
