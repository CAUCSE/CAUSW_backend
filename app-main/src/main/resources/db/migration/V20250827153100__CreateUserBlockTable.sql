...
CREATE TABLE tb_user_block (
    ...
    -- 유저-유저 쌍으로 유니크 인덱스 생성
                               CONSTRAINT uq_block UNIQUE (blocker_id, blockee_id),

    -- 기본 인덱스들
                               INDEX idx_blocker (blocker_id),
                               INDEX idx_blockee (blockee_id),
                               INDEX idx_scope (scope, scope_ref_id),

    -- 추가 성능 최적화 인덱스들
                               INDEX idx_blocker_blockee (blocker_id, blockee_id),
