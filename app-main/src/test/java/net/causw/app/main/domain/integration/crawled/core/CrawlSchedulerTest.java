...
import net.causw.app.main.domain.integration.crawled.service.CrawlFacade;
import net.causw.app.main.domain.integration.crawled.service.CrawledToPostTransferFacade;
...
class CrawlSchedulerTest {
	...
	void runAllSites_shouldTransferAfterCrawling() throws InterruptedException {
		...
		given(crawlFacade.crawlAllEnabled())
		...
		// then
		verify(crawlFacade).crawlAllEnabled();
		verify(crawledToPostTransferFacade).transferToPosts();
		verify(lock).unlock();
	}
}
