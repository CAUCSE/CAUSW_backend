...
public class GrantableRoleValidator extends AbstractValidator {
    ...
    public void validate() {
        ...
        if (!RolePolicy.GRANTABLE_ROLES.contains(this.grantedRole) || !this.grantorRoles.contains(this.grantedRole)) {
        ...
        if (this.grantedRole.equals(Role.PRESIDENT)) {
            if (hasAnyRole(this.granteeRoles, RolePolicy.ROLES_GRANTABLE_BY_PRESIDENT.toArray(new Role[0])))
                return;
        }
        else {
            if (hasAnyRole(this.granteeRoles, Role.COMMON))
                return;
        }
