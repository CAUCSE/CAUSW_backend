...
| 주제 | 문서 |
|------|------|
| Controller, DTO, ApiResponse | [api-layer.md](./api-layer.md) |
...
1. **API 버전**: 현재 표준은 v2 뿐 (v1 은 모두 제거됨). API 경로/디렉터리는 `api/v2/...`, 서비스 레이어는 버전 디렉터리 없이 `service/` 바로 아래.
...
4. `service/implementation/` 디렉터리에 `{Entity}Reader` / `{Entity}Writer` 등 컴포넌트
5. `service/{Entity}Service` (트랜잭션 경계)
6. `service/dto/` 에 Command/Query/Result DTO
