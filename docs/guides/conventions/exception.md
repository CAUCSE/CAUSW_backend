...
이 프로젝트는 모듈 분리에 맞춰 두 개의 예외 시스템이 공존합니다.

| 위치 | 사용 시점 |
|------|----------|
| `global/exception` | `app-main` 외부에서도 쓸 수 있는 가장 기초적인 예외 (`BaseRuntimeException`, `BadRequestException`, `UnauthorizedException` 등) — 주로 v1 / 레거시 |
| `app-main/shared/exception` | `app-main` 내부 표준. v2 신규 코드는 이쪽을 사용 |

신규 비즈니스 예외는 항상 `shared/exception/errorcode/{Domain}ErrorCode` 를 추가해 처리합니다.

## 2. v2 표준: `BaseResponseCode` + `BaseRunTimeV2Exception`
...
| `BadRequestException` (v1 레거시) | 400 | `GlobalErrorCode.BAD_REQUEST` |
| `UnauthorizedException` (v1 레거시) | 401 | `GlobalErrorCode.UNAUTHORIZED` |
| `ForbiddenException` (v1 레거시) | 403 | `GlobalErrorCode.FORBIDDEN` |
| `ServiceUnavailableException` (v1 레거시) | 503 | `GlobalErrorCode.SERVICE_UNAVAILABLE` |
| `BaseRuntimeException` (v1 레거시) | `errorCode.httpStatusCode` | `GlobalErrorCode.BAD_REQUEST` |
...
위치: `shared/exception/GlobalV1ExceptionHandler`

v1 API 에서는 `global.exception.*` 의 `BadRequestException` / `UnauthorizedException` / `ForbiddenException` / `BaseRuntimeException` 을 직접 throw 하는 경우가 있습니다.

신규 코드는 v2 패턴을 따르세요. 굳이 v1 패턴을 흉내내지 마세요.
